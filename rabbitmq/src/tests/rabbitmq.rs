...
fn should_subscribe_to_new_blocks() {
	...
	let value1 = block_receiver.next().unwrap().unwrap();
	let block_str1 = std::str::from_utf8(&value1).unwrap();
	...
	assert_eq!(block_str1, response);

	let value2 = block_receiver.next().unwrap().unwrap();
	let block_str2 = std::str::from_utf8(&value2).unwrap();
	...
	assert_eq!(block_str2, response);

	let value3 = block_receiver.next().unwrap().unwrap();
	let block_str3 = std::str::from_utf8(&value3).unwrap();
...
pub fn should_send_missed_blocks() {
	...
	let value5 = block_receiver.next().unwrap().unwrap();
	let block_str5 = std::str::from_utf8(&value5).unwrap();
	...
	assert_eq!(block_str5, response);

	let value6 = block_receiver.next().unwrap().unwrap();
	let block_str6 = std::str::from_utf8(&value6).unwrap();
